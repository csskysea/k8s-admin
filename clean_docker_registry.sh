...
# /data/docker/registry/docker/registry/v2/repositories/
: ${REGISTRY_URL:=http://127.0.0.1:5000}
: ${REGISTRY_DIR:=/data/docker/registry}
...
keep_last_tags(){
  ...
  local image=$2
  local last_counts=$3
  local last_counts_array=`ll -tr $REPO_DIR/$repo/$image/_manifests/tags|tail -n $last_counts |awk -F ' ' '{print $9}'`
  echo $last_counts_array
...
remove_image_tags() {
  local repo=$1
  local image=$2
  local last_tags_array=$(keep_last_tags)
  ...
  for tag_path in $REPO_DIR/$repo/$image/_manifests/tags/*; do
    ...
    # Do not cleanup execluded tags
    if ! [[ $tag =~ $EXCLUDE_TAGS ]] && ! [[ ${last_tags_array[@]} =~ $tag ]]; then
